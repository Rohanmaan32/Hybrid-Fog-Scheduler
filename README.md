...
Key parameters that can be adjusted in SchedulerMain.py:

Algorithm Comparison Summary
Algorithm	Complexity	Speed	Performance	Energy Efficient
Random	O(n)	✓✓✓	✗	✗
P2C	O(n)	✓✓✓	✓	✓
GA	O(n×p×i)	✓	✓✓	✓✓
EMA	O(n×m)	✓✓	✓✓	✓✓✓
...
Base PSGS	O(n×m)	✓✓	✓✓	✓✓
Hybrid	O(n×s×i)	✗	✓✓✓	✓✓✓
Legend: n=tasks, m=nodes, p=population, i=iterations, s=swarm size, t=temperatures

Performance Results
Typical Output Example
Key Insights
Metaheuristic Algorithms Outperform Heuristics: GA, PSO, and SA-based approaches generally provide better solutions than simple heuristics.

Hybrid Approaches are Effective: Combining multiple algorithms (GA+EMA, Hybrid PSGS+PSO) yields superior results.

Energy-Efficiency Trade-offs: Algorithms optimizing for energy may increase makespan, requiring careful weight tuning.

Deadline Awareness Matters: PSGS-based algorithms effectively handle deadline constraints.

Scalability Considerations: As task count increases, metaheuristic algorithms show better scalability than greedy approaches.

Future Enhancements
 Support for task dependencies and DAG scheduling
 Dynamic node resource allocation
 Real-time task arrival handling
 Multi-objective optimization (Pareto frontier)
 Machine learning-based parameter tuning
 Distributed scheduling framework
 Integration with real fog-cloud platforms
References
Related research on task scheduling in fog-cloud environments:

Energy-aware scheduling in heterogeneous systems
Deadline-constrained task scheduling
Fog computing optimization techniques
Multi-objective optimization in cloud environments
License
This project is part of academic research on task scheduling optimization in fog-cloud environments.

Author
Rohan | BTP Project - Task Scheduling Optimization

Acknowledgments
Developed as part of a Bachelor's Thesis Project (BTP) on optimization techniques for distributed computing environments.
